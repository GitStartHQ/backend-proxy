...
program
  ...
  .option('-s, --secure', 'Listen on https instead of http, a self signed ssl certificate will be used (Default = false)')
...
const handler = createHandler({ proxyUrl, token, useHeaders, debug, tokenName, readOnly, rewrites, secure })
...
if (!secure) {
  ...
  https.createServer({
    key: pkey,
    cert: pcert,
  }, handler).listen(port, errorHandler)
}
